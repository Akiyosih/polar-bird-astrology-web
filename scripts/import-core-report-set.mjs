...
const reports = {
  ...
  "solar-return": {
    ...
    chartPattern: (stamp) => new RegExp(`^chart_wheel_solar_return_core_${escapeRegExp(stamp)}_(?!.*_dark\\.png$).+\\.png$`),
    chartDarkPattern: (stamp) => new RegExp(`^chart_wheel_solar_return_core_${escapeRegExp(stamp)}_.+_dark\\.png$`),
  ...
  "new-moon": {
    ...
    chartPattern: (stamp) => new RegExp(`^chart_wheel_new_moon_core_${escapeRegExp(stamp)}_(?!.*_dark\\.png$).+\\.png$`),
    chartDarkPattern: (stamp) => new RegExp(`^chart_wheel_new_moon_core_${escapeRegExp(stamp)}_.+_dark\\.png$`),
...
function usage() {
  return [
    ...
    "and matching dark chart wheel PNGs generated by scripts.render_chart_wheel."
  ].join("\n");
}
...
async function findDarkChart(sourceDir, config, stamp) {
  const matcher = config.chartDarkPattern(stamp);
  const files = await walkFiles(sourceDir);
  const candidates = [];
  ...
  candidates.sort((a, b) => a.nestingPenalty - b.nestingPenalty || b.mtimeMs - a.mtimeMs);
  if (candidates.length === 0) {
    throw new Error(`No dark chart wheel PNG found for stamp ${stamp}. Render it with scripts.render_chart_wheel --theme dark before importing.`);
  }
...
async function importReport(key, sourceDir, pruneOldAssets, assetVersion) {
  ...
  const sourceChartDark = await findDarkChart(sourceDir, config, source.stamp);
  const publicPdf = config.pdfName(date);
  const publicChart = config.chartName(date);
  const publicChartDark = config.chartDarkName(date);
  ...
  const chartDarkTarget = path.join(chartDir, publicChartDark);
  const contentTarget = path.join(root, config.contentFile);

  for (const target of [pdfTarget, chartTarget, chartDarkTarget, contentTarget]) {
    assertInside(root, target);
  }
  ...
  await copyFile(sourceChartDark, chartDarkTarget);
  ...
  if (pruneOldAssets) {
    ...
    await pruneMatching(chartDir, config.chartDarkPublicPattern, publicChartDark);
  }
